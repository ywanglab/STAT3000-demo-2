...
3. **Create and include a plot image**  
    ...
    ```{r}
        ...
        plot(cars)
        dev.off() # close the current device and fall backs to "null derice 1" (no device open)
    ```
...
4. **Define a function and compute real roots**
   ...
   * If the discriminant is negative, print a message like `"No real roots"`.
    ...
    ```{r}
     ...
     if (disc <0 ) {
       cat("The discriminant is negative, hence no real roots")
       } else {
        x1<- 1/(2*a)*(-b+sqrt(disc))
        x2 <- 1/(2*a)*(-b- sqrt(disc))
        cat("The roots are x1 =", x1, ", x2 = ", x2)
      }
    ```
...
9. **Switch to an absolute path and test portability**
   ...
   Render it once to confirm it works.
    ...
    coefs <- scan(file.path(getwd(), "../data/coefs.txt")) #read numbers/string and return a vector
